...
def process_mosaics(site_slide_path, site_mosaic_path):
    pool = mp.Pool(4)
    total = len(glob.glob(os.path.join(site_mosaic_path, "*", "*", "coord", "*")))
    progress = 0

    for mosaic in glob.glob(os.path.join(site_mosaic_path, "*", "*", "coord", "*")):
        mosaic = mosaic.replace("\\", "/")
        print("{}/{} Process mosaic: {}".format(progress, total, mosaic), flush=True)
        ...
        match = re.match(r"(\d+)x", resolution)
        magnification_level = int(match.group(1))
        patch_size = int(51.2 * magnification_level)

        print("Original mosaic size:", len(coords), flush=True)

        # Remove the white artifacts
        slide_path = os.path.join(site_slide_path, diagnosis, resolution, slide_id + ".svs")
        t_start = time.time()
        ...
        print("Clean mosaic size:", len(coord_clean), flush=True)
        print("Removal takes: ", time.time() - t_start, flush=True)

        save_path_clean = os.path.join(site_mosaic_path, diagnosis,
                                       resolution, "coord_clean")
        save_path_artifacts = os.path.join(site_mosaic_path, diagnosis,
...
if __name__ == "__main__":
    ...
    process_mosaics(args.site_slide_path, args.site_mosaic_path)
