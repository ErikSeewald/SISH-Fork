...
import os
import pdb
from SISH_Fork.wsi_core.util_classes import Mosaic_Canvas
...
def isBlackPatch_oncopanel(patch, lower_bound, upper_bound, percentage=0.05):
    patch_size = patch.size[0]
    patch_hsv = cv2.cvtColor(np.array(patch), cv2.COLOR_RGB2HSV)
    mask = cv2.inRange(patch_hsv, lower_bound, upper_bound)
    mask //= 255
    area_ratio = np.sum(mask) / (patch_size * patch_size)
    if area_ratio >= percentage:
        return True
def isGreenPatch_oncopanel(patch, lower_bound=(80, 120, 0), upper_bound=(110, 255, 255), percentage=0.05):
    patch_size = patch.size[0]
    patch_hsv = cv2.cvtColor(np.array(patch), cv2.COLOR_RGB2HSV)
    mask = cv2.inRange(patch_hsv, lower_bound, upper_bound)
    mask //= 255
...
def isBluePatch_oncopanel(patch, lower_bound=(90, 100, 0), upper_bound=(130, 255, 255), percentage=0.05):
    patch_size = patch.size[0]
    patch_hsv = cv2.cvtColor(np.array(patch), cv2.COLOR_RGB2HSV)
    mask = cv2.inRange(patch_hsv, lower_bound, upper_bound)
    mask //= 255
...
def SamplePatches(coords_file_path, save_file_path, wsi_object, 
    ...
    for idx in indices:
        ...
        if isBlackPatch_S(patch, rgbThresh=20, percentage=0.05) or isWhitePatch_S(patch, rgbThresh=220, percentage=0.25):
            continue
        # if isBlackPatch_oncopanel(patch, lower_bound=(0, 50, 50), upper_bound=(25, 110, 110))\
        # or isBlackPatch_oncopanel(patch, lower_bound=(0, 95, 20), upper_bound=(15, 125, 50))\
        # or isBlackPatch_oncopanel(patch, lower_bound=(110, 40, 90), upper_bound=(140, 70, 170))\
        # or isBlackPatch_oncopanel(patch, lower_bound=(110, 40, 20), upper_bound=(140, 90, 100)):
        #     continue
        # if isGreenPatch_oncopanel(patch) or isGreenPatch_oncopanel(patch, lower_bound=(70, 90, 160), upper_bound=(100, 129, 255)):
        #     continue
        # if isBluePatch_oncopanel(patch) or isBluePatch_oncopanel(patch, lower_bound=(90, 70, 170), upper_bound=(120, 100, 255)):
        #     continue
        if stitch:
